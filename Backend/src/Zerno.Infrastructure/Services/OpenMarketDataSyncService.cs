    ...
    ILogger<OpenMarketDataSyncService> logger) : IMarketDataSyncService
{
    ...
    private async Task SyncNewsAsync(CancellationToken cancellationToken)
    {
        ...
        var desired = new[]
        {
            new FeedItem("Редакция ЗерноАгроМир", "Смоленские зерновые хозяйства нарастили отгрузки по внутренним контрактам", "В ЦФО ускорилась логистика по пшенице 3 и 4 класса, а трейдеры фиксируют устойчивый спрос со стороны мукомольных предприятий.", "Главные новости", "Россия", "Пшеница", "ЦФО", "news", DateTime.UtcNow.AddHours(-3)),
