...
export function MarketplacePage() {
  ...
  const searchOptions = useMemo(() => {
    const sellers = users.map((user) => ({ value: user.name, label: `Участник: ${user.name}` }));
  ...
  return (
    <Space direction="vertical" size={24} style={{ width: '100%' }}>
        ...
        <Row gutter={[24, 24]} align="middle">
          <Col xs={24} xl={14}>
            <Space direction="vertical" size={16} style={{ width: '100%' }}>
              <Tag color="gold">Торговая площадка</Tag>
              <Typography.Title level={1}>Зерно, техника и услуги в одном понятном каталоге</Typography.Title>
              <Typography.Paragraph className="lead-text">
                Смотрите проверенные предложения, сравнивайте цены, открывайте карточки лотов и сразу переходите к корзине или запросу условий.
              </Typography.Paragraph>
              <Space wrap>
                <Button type="primary" onClick={resetFilters}>Сбросить фильтры</Button>
                <Button onClick={() => setSearchParams({ tab: 'grain' })}>Показать зерно</Button>
                <Button onClick={() => setSearchParams({ tab: 'equipment' })}>Показать технику</Button>
              </Space>
            </Space>
          </Col>
          <Col xs={24} xl={10}>
            <Row gutter={[12, 12]}>
              <Col xs={24} sm={8} xl={8}>
                <Card className="metric-card">
                  <Typography.Text className="metric-title">Зерновые лоты</Typography.Text>
                  <Typography.Title level={3}>{grainLots.length}</Typography.Title>
                </Card>
              </Col>
              <Col xs={24} sm={8} xl={8}>
                <Card className="metric-card">
                  <Typography.Text className="metric-title">Лоты техники</Typography.Text>
                  <Typography.Title level={3}>{equipmentLots.length}</Typography.Title>
                </Card>
              </Col>
              <Col xs={24} sm={8} xl={8}>
                <Card className="metric-card">
                  <Typography.Text className="metric-title">Услуги</Typography.Text>
                  <Typography.Title level={3}>{serviceOffers.length}</Typography.Title>
                </Card>
              </Col>
            </Row>
          </Col>
      ...
      <Card title="Режимы выдачи">
        <Segmented
          ...
          options={[
            ...
            { label: 'Проверенные участники', value: 'verified' },
          ]}
          block
        />
      </Card>
      ...
      <Card title="Поиск и фильтры" extra={<Button onClick={resetFilters}>Сбросить фильтры</Button>}>
        <Space direction="vertical" size={16} style={{ width: '100%' }}>
          <AutoComplete value={query} options={searchOptions} onChange={setQuery} style={{ width: '100%' }} placeholder="Поиск по культурам, регионам, участникам и типам лотов" />
          ...
          <Space size={24} wrap>
            ...
            <Space><Switch checked={verifiedOnly} onChange={setVerifiedOnly} /><Typography.Text>Только проверенные участники</Typography.Text></Space>
          </Space>
        </Space>
      </Card>
      ...
      <Row gutter={[24, 24]}>
        <Col xs={24}>
          <Card title={<Space><FilterOutlined />Каталог предложений</Space>}>
            <Space direction="vertical" size={16} style={{ width: '100%' }}>
              <Segmented
                value={tab}
                onChange={(value) => {
                ...
                options={[{ label: 'Зерно', value: 'grain' }, { label: 'Техника', value: 'equipment' }, { label: 'Услуги', value: 'services' }]}
                block
              />
                ...
                <Space direction="vertical" size={16} style={{ width: '100%' }}>
                  {filteredServices.map((item) => (
                    <Card key={item.id} className="lot-card">
                      <Row gutter={[16, 16]}>
                        <Col xs={24} md={8}><img src={serviceImageMap[item.id] ?? '/images/thematic/image_01.jpg'} alt={item.title} className="lot-image-thumb" /></Col>
                        <Col xs={24} md={16}>
...
function GrainLotCard({ lot, isVerified, onAdd }: { lot: GrainLot; isVerified: boolean; onAdd: () => void }) {
  ...
  return (
    <Card className="lot-card clickable-card" onClick={() => navigate(`/marketplace/lot/${lot.id}`)} role="button" tabIndex={0}>
      <Row gutter={[16, 16]}>
        <Col xs={24} xl={6}><img src={lot.coverImageUrl ?? lotImageMap[lot.id]?.[0] ?? '/images/thematic/image_01.jpg'} alt={lot.title} className="lot-image-thumb" /></Col>
        <Col xs={24} xl={11}>
          <Space direction="vertical" size={10} style={{ width: '100%' }}>
            <Space wrap>
              <Tag color="green">{lot.grainType}</Tag>
              <Tag color="blue">{lot.grade}</Tag>
              <Tag color={isVerified ? 'success' : 'warning'} icon={<SafetyCertificateOutlined />}>{isVerified ? 'Проверенный участник' : 'Профиль без проверки'}</Tag>
            ...
            <div className="grain-grid">
              ...
              <InfoCell label="Участник" value={lot.sellerName} />
            ...
            <Space wrap>
              ...
              <Tag>{lot.hasOwnTransport ? 'Доставка по заявке' : 'Самовывоз'}</Tag>
              <Tag>Оплата: безналичный расчет</Tag>
            </Space>
            <Divider />
            <Space wrap>
              <Button type="primary" icon={<ShoppingCartOutlined />} onClick={(e) => { e.stopPropagation(); onAdd(); }}>Купить</Button>
              <Button onClick={(e) => { e.stopPropagation(); navigate(`/marketplace/lot/${lot.id}`); }}>Запросить условия</Button>
              <Button icon={<HeartOutlined />} onClick={(e) => e.stopPropagation()}>Добавить в избранное</Button>
              <Button onClick={(e) => e.stopPropagation()}>Сравнить</Button>
            </Space>
          </Space>
        </Col>
        ...
        <Col xs={24} xl={7}>
          <Card className="auction-card">
            <Space direction="vertical" style={{ width: '100%' }}>
              ...
              {lot.auctionEnabled ? (
                <>
                  <Typography.Text>Текущая ставка: {formatRublesPerTon(lot.pricePerTon + 300)}</Typography.Text>
...
function EquipmentLotCard({ lot, isVerified, onAdd }: { lot: EquipmentLot; isVerified: boolean; onAdd: () => void }) {
  ...
  return (
    <Card className="lot-card clickable-card" onClick={() => navigate(`/marketplace/lot/${lot.id}`)} role="button" tabIndex={0}>
      <Row gutter={[16, 16]}>
        <Col xs={24} xl={8}><img src={lot.coverImageUrl ?? lotImageMap[lot.id]?.[0] ?? '/images/thematic/image_01.jpg'} alt={lot.title} className="lot-image-thumb" /></Col>
        <Col xs={24} xl={16}>
          <Space direction="vertical" size={10} style={{ width: '100%' }}>
            <Space wrap>
              ...
              <Tag color={isVerified ? 'success' : 'warning'} icon={<CheckCircleOutlined />}>{isVerified ? 'Проверенный участник' : 'Без проверки'}</Tag>
            </Space>
            <Typography.Title level={3}>{lot.title}</Typography.Title>
            <div className="grain-grid">
            ...
            <Typography.Paragraph>{lot.description}</Typography.Paragraph>
            <Space wrap>
              <Button type="primary" onClick={(e) => { e.stopPropagation(); onAdd(); }}>Запросить предложение</Button>
              <Button onClick={(e) => { e.stopPropagation(); navigate(`/marketplace/lot/${lot.id}`); }}>Открыть карточку</Button>
              <Button icon={<HeartOutlined />} onClick={(e) => e.stopPropagation()}>В избранное</Button>
            </Space>
          </Space>
        </Col>
      </Row>
