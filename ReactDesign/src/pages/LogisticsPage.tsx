...
const deliveryModes: { value: DeliveryMode; label: string }[] = [
  ...
  { value: 'seller_delivery', label: 'Доставка участником' },
    ...
    <Space direction="vertical" size={24} style={{ width: '100%' }}>
      <Card className="section-card">
        ...
        <Typography.Paragraph className="lead-text">
          ...
          ж/д перевозка, доставка участником и партнерская перевозка.
      ...
      <Row gutter={[24, 24]}>
        ...
        <Col xs={24} xl={13}>
          <Card title="Результат расчета" extra={quoteReady ? <Tag color="green">Расчет готов</Tag> : <Tag>Предпросмотр</Tag>}>
            <Row gutter={[16, 16]}>
              <Col xs={24} md={8}>
                <Statistic title="Итоговая стоимость" value={quote.price} formatter={(value) => formatRubles(Number(value))} />
              </Col>
              <Col xs={24} md={8}>
                <Statistic title="Ориентировочный срок" value={quote.etaDays} suffix="дня" />
              </Col>
              <Col xs={24} md={8}>
                <Statistic title="Цена за тонну" value={Math.round(quote.price / Math.max(1, volume))} formatter={(value) => formatRublesPerTon(Number(value))} />
              </Col>
            </Row>
            ...
            <Card className="nested-card" style={{ marginTop: 16 }}>
              <Space direction="vertical" size={8}>
                ...
                <Typography.Text>Базовая перевозка: {formatRubles(Math.round(quote.price * 0.74))}</Typography.Text>
                <Typography.Text>Погрузка/разгрузка: {formatRubles(Math.round(quote.price * 0.12))}</Typography.Text>
                <Typography.Text>Страхование: {formatRubles(Math.round(quote.price * 0.06))}</Typography.Text>
                <Typography.Text>Оформление и сопровождение: {formatRubles(Math.round(quote.price * 0.08))}</Typography.Text>
              </Space>
            </Card>
